    ...
    <div className="rounded-2xl border border-[#E8E3D9] bg-white p-4">
      ...
      >
        <h2 className="text-sm font-semibold uppercase tracking-wide text-[#8A8478]">
          История{sorted.length > 0 ? ` (${sorted.length})` : ''}
        </h2>
        {isOpen ? (
        ...
        (sorted.length === 0 ? (
          ...
          <div className="mt-3 flex flex-col divide-y divide-[#E8E3D9]">
            {sorted.map((t) => {
              ...
              const title = isTransfer ? 'Перевод' : t.category;
              const currency = isTransfer
                ? (accounts[t.fromAccount!]?.currency ?? 'EUR')
                : (accounts[t.account!]?.currency ?? 'EUR');
