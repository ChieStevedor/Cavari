...
Нативний Android-застосунок (Kotlin) для голосових нотаток:
...
(`gpt-4o-transcribe-diarize`, мова визначається автоматично — українська,
англійська чи їх суміш), збережену локально в Room database. Довгі записи
(розмови, дзвінки) автоматично розпізнають окремих мовців.
...
2. **Запис і транскрипція** — `AudioRecorder` (безперервний `AudioRecord` →
   ...
   мова визначається автоматично, розпізнає окремих мовців).
...
- Увімкніть перемикач "Прослуховування активне", скажіть **"Hey, Naomi"** з
  вимкненим екраном/іншого застосунку → коротка вібрація підтвердить старт
  запису, скажіть нотатку і замовкніть — запис сам зупиниться після паузи
  (подвійна вібрація) і збереже нотатку без жодного дотику до екрана.
- Кожна нотатка в списку має дві іконки: **⧉** копіює текст у буфер обміну,
  **✕** видаляє нотатку.
- Раз на день (близько 19:00) `DailyDigestWorker` збирає нотатки за той
  день і зберігає `.txt`-файл у `Downloads`, зі сповіщенням і кнопкою
  "Поділитися" (працює з Android 10+, через MediaStore).
